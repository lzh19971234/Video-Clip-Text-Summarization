...
def getstart_v2(fp, retain, subtitles, times, word_embeddings):
    ...
    logger.info(get_time())
    logger.info('demo = ExtractableAutomaticSummary(article)')

    demo = ExtractableAutomaticSummary(article, word_embeddings)

    logger.info(get_time())
    logger.info('demo.calculate()')

    demo.calculate()

    logger.info(get_time())
    logger.info('ans = demo.get_abstract(nums)')
...
def get_summary(filename,text_name, retain, times, word_embeddings):
    """ Abstract function
    ...
    regions = getstart_v2(text_name, retain, subtitles, times, word_embeddings)
    logger.info('summary end')
    logger.info(get_time())
    logger.info('splicing start')
    summary = create_summary(filename, regions)
    base, ext = os.path.splitext(filename)
    video_name = "{0}v2".format(base) + str(random.randint(1, 100000)) + str(datetime.datetime.now().strftime('%y%m%d%H%M%S'))
